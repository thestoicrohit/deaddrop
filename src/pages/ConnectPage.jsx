...
const DEMO_ADDR   = '0x3f7a9b2d4e1c8f5a6b0d9e2c7f4a1b8e5c2d9f6a'
...
function WalletButton({ icon, label, sublabel, onClick, disabled }) {
  return (
    <motion.button
      onClick={onClick}
      disabled={disabled}
...
export default function ConnectPage() {
  ...
  const { connectWallet, setDisplayName, setProfilePhoto, setSafePin,
          completeOnboarding, onboardingComplete, setDemoMode } = useAppStore()
  ...
  // When wagmi successfully connects, advance the flow
  useEffect(() => {
  ...
  const handleMetaMask = () => {
    if (!window.ethereum) {
      toast.error('MetaMask not found — please install it.', { duration: 4000 })
      ...
      { connector: injected() },
      {
        onError: (err) => {
          if (err.message?.includes('rejected')) {
            ...
            toast.error(err.shortMessage || err.message || 'Connection failed.')
          }
        },
      }
    )
  ...
  const handleDemo = () => {
    setDemoMode(true)
    connectWallet(DEMO_ADDR)
    toast('Running in Demo Mode — no real wallet needed.', { icon: '🧪' })
    if (onboardingComplete) navigate('/dashboard')
    else setStep(1)
  }
                  ...
                  {isConnecting ? (
                    ...
                    <>
                      ...
                      <WalletButton
                        icon="🦊"
                        label="MetaMask"
                        sublabel="Connect with your browser wallet"
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={handleProfileNext} className="btn-primary w-full">Continue</button>
                </div>
              </motion.div>
            )}
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={handlePinNext} className="btn-primary w-full">Enter My Vault</button>
